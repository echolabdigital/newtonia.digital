...
function require_tenant(): array {
    ...
    if ($t['status'] === 'suspended') {
        _hermes_access_blocked('Conta suspensa', 'Sua conta está suspensa. Verifique sua cobrança em <a href="/app/billing.php">Plano e cobranças</a> ou entre em contato: <strong>contato@hermesb2b.co</strong>');
    }
    if ($t['status'] === 'cancelled') {
        _hermes_access_blocked('Conta cancelada', 'Esta conta foi cancelada. Pra reativar, entre em contato: <strong>contato@hermesb2b.co</strong>');
    }
    return $t;
}

// ─── Páginas de erro com identidade HERMES.b2b ──────────────────────────────
...
function _hermes_error_page(string $title, string $message, string $extra, array $actions): void {
    ...
    echo '<title>' . htmlspecialchars(strip_tags($title)) . ' — HERMES.b2b</title>';
    ...
    echo '<style>
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .brand-icon{width:42px;height:42px;border-radius:10px;background:#10b981;color:#fff;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(16,185,129,.25)}
        .brand-text{font-family:Geist Mono,monospace;font-weight:700;font-size:1.1rem;line-height:1.1}
        .brand-text .b2b{color:#10b981;font-size:.76em}
        ...
        .card a:not(.btn){color:#10b981;text-decoration:none;font-weight:600}
    ...
    echo '<a class="brand" href="/"><div class="brand-icon"><svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M5 7l5 5-5 5"/><line x1="13" y1="17" x2="20" y2="17"/></svg></div><div class="brand-text">HERMES<span class="b2b">.b2b</span></div></a>';
    ...
    echo '</div></div><div class="foot">hermesb2b.co · by echo_lab</div></div></body></html>';
    exit;
}
